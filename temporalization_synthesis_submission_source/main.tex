...
\section{Atemporal access and the conditional expansion class}

The ECTR packet sharpens the relation between the atemporal carrier and a
time-dependent four-dimensional readout.  The source-frozen access coordinate
\begin{equation}
 u(\Gamma)=\int_\Gamma\eta_\tau
 \label{eq:synthesis-ectr-access}
\end{equation}
is a simultaneous path invariant, not a stored trajectory or a primitive
Parent clock.  Suppose, however, that the frozen seed--body packet and one
post-body Legendre--Riesz coevaluation supply a common positive operator
\begin{equation}
 G={K_{\rm src}\over2A_*}=k_{c,O}^2A_{0,M}^{-1},
 \qquad F_{Q,O}(a)=(I+a^2G)^{-1/2},
 \label{eq:synthesis-ectr-lock}
\end{equation}
and that the same scale $a$ belongs to an independently metric-active
coframe.  Pulling the path to a positively calibrated observer clock gives
\begin{equation}
 \boxed{H_O={\Gamma_{U,O}\over\mathcal E_Q(a)}},\qquad
 \Gamma_{U,O}=-d_{\tau_O}\log\det F_{Q,O},\quad
 \mathcal E_Q=-\partial_{\log a}\log\det F_{Q,O}.
 \label{eq:synthesis-ectr-classifier}
\end{equation}
On a positive nonzero R1 ray, $\Gamma_{U,O}>0$ and the Q--metric branch
expands.  The readout therefore evolves in observer time even though its
Parent access datum is atemporal.

This is a common-source rather than an expansion-causes-terminals statement.
The metric-active $a_O$ and the expansion-conditioned access weights are
co-descendants of the frozen packet. Once the law is selected, $a_O$ can
modulate which typed leg dominates; the observed expansion cannot select the
operator, quantizer or quantum/gravity semantics backward.

This is not an unconditional temporalization theorem.  Zero access gives a
static readout, assigning the loss to the complementary terminal leg reverses
the sign, and missing source co-registration leaves the context unclassified.
The sign becomes common only across compatible comoving observers on one
oriented globally hyperbolic leaf.  Consequently the present result proves a
nonempty expanding sector of the atemporal architecture, while universal
expansion, literal Parent evolution, our universe's source-class membership,
acceleration and a definite $H(z)$ remain open.
...
The current finite laboratory is a reproducible implementation, not a new Nature-selection theorem. Genetic generations are computational search indices, not Parent occurrence or observer time. The propagation example supplies its readout-time chart and kinetic law independently. Neither its unit chart speed nor a finite acceptance cell establishes a universal time atom, physical clock calibration or temporal Nature occupation.
...
The current observer audit separates a conditional forward construction from physical source selection. A frozen body, occupied observer incidence, preparation, complete instrument and stable resolution can determine records without selecting those inputs from Nature. Equal effects do not fix postmeasurement states, and symmetry within a chosen interaction family does not exclude other symmetric couplings. The supplied oscillator clock and energy conversion are downstream premises. Neither a covariance nor ground-state minimization derives observer time or turns the atemporal parent into a dynamical memory.
\subsection{Gradient occurrence, induced resistance and inertial completion}
The temporal working branch must distinguish three conditional laws. Original
EOCC selects $x'=-\Gamma^{-1}\nabla U$ with $\Gamma>0$; at a strict minimum
its linearized spectrum is real and negative. In contrast, a supplied
reversible-plus-loss completion on $(x,\pi)$ gives
\[
 Mx''+Dx'+\nabla U=0,\qquad M>0,\quad D\succeq0.
\]
Its energy decreases, but underdamped poles exclude an identification with
the original gradient law by an invertible change of state coordinates.
The kinetic carrier and reversible pairing are extra source data, not a
renaming of the same occurrence prescription.

For a declared base drag $D_Bb'+Ab+Cx=0$ and algebraic body constraint
$Hx+C^Tb=j$, zero-initial-state elimination gives
\[
 K(p)=H-C^T(A+pD_B)^{-1}C
 =H_{\rm eff}+pG_{\rm ind}+O(p^2),
\]
\[
 H_{\rm eff}=H-C^TA^{-1}C,\qquad
 G_{\rm ind}=C^TA^{-1}D_BA^{-1}C.
\]
Under injective coupling and a controlled low-frequency expansion,
$G_{\rm ind}$ is the resistance, not its inverse mobility. For scalar
$A=a,H=h,C=c,D_B=d$ with $ah>c^2>0$, the exact constant-load rate is
$(ah-c^2)/(dh)$, whereas the reduced EOCC rate is
$a(ah-c^2)/(c^2d)$. Their ratio is $ah/c^2$ although the equilibria agree.
With base inertia $m>0$, define
\[
 k_B=a-c^2/h,\quad r_0=k_B/d,\quad
 \delta=mk_B/d^2,\quad \epsilon=(ah-c^2)/c^2.
\]
For $0<\delta<1/4$ the slow rate is
$r_s/r_0=2/(1+\sqrt{1-4\delta})$, while $r_E/r_0=1+\epsilon$.
Both approximation parameters, the initial layer and preparation matter.
Matching one late decay rate does not identify the complete response.

The full equal-weight outgoing $A_3$ carrier provides a further adverse
control: its static loading is not automatically neutral, and matched
root/bulk inertia with a positive root pin retains an above-band localized
mode. Energetic stability therefore does not imply generic complete settling.
These statements belong to the explicitly enlarged hypothetical formation
branch. They neither put memory into the atemporal parent nor derive Nature
occupation or observer time; the latter still needs source-dependent
$Q_{OS}$ and independent clock calibration.

\paragraph{Dependency and claim ownership.}
Foundation VI and the body--readout technical companion own the finite instrument/access proof; Foundation IV and TD-0 retain physical source selection; Foundation V, the temporal synthesis and seed--body companion own the gradient/inertial distinction. This paper inherits only the stated specialization. Canonical assumptions, countermodels and calibration audits are in \url{https://github.com/tau-core-research/tau-core-theory/blob/main/docs/tau_rqm_instrument_to_record_bridge_001.md}. The temporal source audit is \url{https://github.com/tau-core-research/tau-core-theory/blob/main/docs/tau_core_temporal_seed_base_morphogenesis_branch_001.md}, sections 628--630. No physical seed, Nature occupation or Tau-exclusive observation is established by this update. Existing empirical endpoint definitions and scores are unchanged.
